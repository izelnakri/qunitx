...
export default async function setupBrowser(config = {
  ...
  let server = connections.server || await setupWebServer(config, cachedContent);

  if (!connections.server) {
    connections.server = await bindServerToPort(config, server);
  }

  connections.browser = connections.browser || await Puppeteer.launch({
    executablePath: process.env.CHROME_BIN || null,
    headless: true,
    args: ['--no-sandbox', '--disable-gpu', '--remote-debugging-port=0', '--window-size=1440,900']
  });
  let page = connections.page || await connections.browser.newPage();
  ...
  await Promise.all(arrayOfHTMLs.map(async (dynamicHTMLPath) => {
    ...
    await fs.rm(`${config.projectRoot}/${config.output}/${relativePath}`, { force: true });
  }));
  ...
  await Promise.all(arrayOfHTMLs.map(async (htmlPath) => {
    ...
    await runTestInsideHTMLFile(relativePath, connections, config) // NOTE: we are blocking each html here on purpose
  }));
...
async function runTestInsideHTMLFile(filePath, { page, server }, config) {
  let QUNIT_RESULT;
  try {
    console.log('#', kleur.blue(`Running: http://localhost:${server.config.port}${filePath}`));
