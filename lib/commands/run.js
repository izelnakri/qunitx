...
export default async function(config) {
  if (config.browser) {
    ...
    await runTestsInBrowser(config.userFileOrFolderInputs, config, cachedContent, connections);

    if (config.watch) {
      logWatcherAndKeyboardShortcutInfo(config, connections.server);

      await fileWatcher(config.userFileOrFolderInputs, async (file) => {
        await runTestsInBrowser([file], config, cachedContent, connections);
      }, (path, event) => connections.server.publish('refresh', 'refresh'), config);
    }
  } else {
    ...
    await runTestsInNode(config.userFileOrFolderInputs, config);
