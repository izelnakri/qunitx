...
export default async function(options = {
  httpPort: 4200, debug: false, wsPort: 65511, watch: false
}, boilerplateQunit='') {
  // TODO: make these async
  let QUnitCSS = (await fs.readFile(`${process.cwd()}/node_modules/qunit/qunit/qunit.css`)).toString();
...
<html>
  ...
  <body>
    ...
    <script>
      ...
      window.socket = new WebSocket('ws://localhost:${options.wsPort}');
  ...
  await new Promise((resolve, reject) => {
    server.listen(options.httpPort, () => {
      if (options.watch || options.debug) {
        console.log(`http server running on port ${options.httpPort}`);
      }
  ...
  await page.goto(`http://localhost:${options.httpPort}/index.html`, { timeout: 0 });
  await page.waitForFunction(`window.testTimeout >= ${options.timeout}`, { timeout: 0 });
