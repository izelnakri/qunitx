...
export default async function runTestsInNode(userFileOrFolderInputs, config) {
  ...
  try {
    ...
    const fsTree = await parseFsInputs(userFileOrFolderInputs, {}, async (targetPath, fsEntry) => {
      count = count + 1;
