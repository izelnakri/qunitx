  ...
  if (config.browser) {
    ...
    if (config.watch) {
      ...
      await fileWatcher(
        ...
        async (file) => {
          await runTestsInBrowser([file], config, cachedContent, { server, page, browser }) // TODO: check if its html then run html otherwise run all html, how??
        },
        (path, event) => server.publish('refresh', `${event}: ${path}`),
        config
      );
    }
  } else {
