  ...
  let fileWatchers = testFileLookupPaths.reduce((watcher, watchPath) => {
    return Object.assign(watcher, {
      [watchPath]: chokidar.watch(watchPath, { ignoreInitial: true }).on('all', (event, path) => {
        if (pathIsForBuild(path)) {
          console.log(path);
          if (event === 'add') {
            config.fsTree[path] = null;
          } else if (event === 'unlink') {
            delete config.fsTree[path];
            console.log(config.fsTree);
          ...
          if (!global.chokidarBuild) {
            ...
            let result = pathIsForBuild(path) ? onEventFunc(path) : null;
            ...
            result
              .then(() => {
                onFinishFunc ? onFinishFunc(path, event) : null;
              })
              .catch(() => {
...
function pathIsForBuild(path) {
  return ['.js', '.ts'].some((extension) => path.endsWith(extension));
}
