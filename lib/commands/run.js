...
export default async function(config) {
  if (config.browser) {
    ...
    let cachedContent = await buildCachedContentTree(config.projectRoot, config.htmlPaths);

    await writeOutputStaticFiles(config, cachedContent);

    const connections = await runTestsInBrowser(config.fileOrFolderInputs, config, cachedContent);
...
async function buildCachedContentTree(projectRoot, htmlPaths) {
  ...
  return await addCachedContentMainHTML(projectRoot, cachedContent);
}
