...
export default async function(config) {
  if (config.browser) {
    ...
    await runTestsInBrowser(Object.keys(config.fsTree), config, cachedContent, connections);
    ...
    if (config.watch) {
      ...
      await fileWatcher(
        ...
        async (event, file) => {
          if (event === 'addDir') {
            return;
          } else if (['add', 'unlink', 'unlinkDir'].includes(event)) { // TODO: this changes / url to only include added test, change that!
            return await runTestsInBrowser(Object.keys(config.fsTree), config, cachedContent, connections);
          }

          await runTestsInBrowser([file], config, cachedContent, connections); // TODO: this changes / url to only include added test, change that!
        },
