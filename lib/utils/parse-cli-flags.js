...
// { fileOrFolderInputs: [], browser: true, debug: true, watch: true, failFast: true, reporter: '', coverage: true , htmlPaths: [], output }
export default async function() {
  const providedFlags = process.argv.slice(2).reduce((result, arg) => {
    if (arg.startsWith('--browser')) {
      ...
      if (result.htmlPaths) {
        result.htmlPaths.push(`${process.cwd()}/${arg}`);
      } else {
        result.htmlPaths = [`${process.cwd()}/${arg}`];
      }
    ...
    result.fileOrFolderInputs.push(`${process.cwd()}/${arg}`);
