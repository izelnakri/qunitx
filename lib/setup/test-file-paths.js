...
export default function setupTestFilePaths(projectRoot, userFileOrFolderInputs) { // NOTE: very complex algorithm, order is very important
  ...
  let result = folders.reduce((folderResult, folder, folderIndex) => {
    if (!pathIsIncludedInPaths(folders, folder, folderIndex)) {
  ...
  filesWithGlob.forEach((file, fileIndex) => {
    if (
      ...
      !pathIsIncludedInPaths(filesWithGlob, file, fileIndex)
    ) {
      result.push(file);
    }
  });
...
function pathIsIncludedInPaths(paths, targetPath, targetPathIndex) {
