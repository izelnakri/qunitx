...
export default async function setupBrowser(config = {
  ...
  await Promise.all(arrayOfHTMLs.map(async (htmlPath) => {
    await runTestInsideHTMLFile(htmlPath, server, page, config.timeout) // NOTE: we are blocking each html here on purpose
  }));
...
async function runTestInsideHTMLFile(filePath, server, page, timeout) {
  let QUNIT_RESULT;
  try {
