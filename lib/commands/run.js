...
export default async function(config) {
  if (config.browser) {
    ...
    if (mainHTMLPath) {
      ...
      cachedContent.mainHTML = { filePath: `${config.projectRoot}/index.html`, html };
      findInternalAssetsFromHTML(html).forEach((key) => {
        cachedContent.assets.add(normalizeInternalAssetPathFromHTML(config.projectRoot, key))
      });
    }
...
function normalizeInternalAssetPathFromHTML(projectRoot, assetPath, htmlPath) { // NOTE: maybe normalize ..
  ...
  return assetPath.startsWith('./')
    ? `${currentDirectory}/${assetPath.slice(2)}`
